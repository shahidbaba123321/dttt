(function() {
    ...
    // Ensure PricingManager is not already defined
    ...
    class PricingManager {
        ...
        // HTTP Client Determination Method
        determineHttpClient() {
        ...
        initializeDOMElements() {
            try {
                ...
                if (!this.plansContainer || !this.addPlanBtn || !this.modalContainer || !this.notificationContainer) {
                    console.warn('Some DOM elements are missing', {
                        plansContainer: !!this.plansContainer,
                        addPlanBtn: !!this.addPlanBtn,
        ...
        async initialize() {
            try {
                ...
                console.log('PricingManager initialization complete');
            } catch (error) {
                console.error('Initialization failed:', error);
                this.showNotification('Failed to initialize Pricing Manager', 'error');
            }
        }
                ...
                response.data.forEach(module => {
                    ...
                    moduleElement.innerHTML = `
                        <input 
                            ...
                            value="${module._id}"
                            ${this.selectedModules.some(selected => selected.id === module._id) ? 'checked' : ''}
                        >
        ...
        validateModuleSelection() {
            ...
            const selectedCount = selectedModuleCheckboxes.length;

            // Validate module selection
            const minModules = 1;
            const maxModules = 10;

            if (selectedCount < minModules) {
                this.showNotification(`Please select at least ${minModules} module(s)`, 'warning');
                return false;
            }

            if (selectedCount > maxModules) {
                this.showNotification(`You can select a maximum of ${maxModules} modules`, 'warning');
                return false;
            }
        ...
        displaySelectedModulesList() {
            ...
            try {
                ...
                const planData = {
                    ...
                    status: planStatus,
                    modules: this.selectedModules.map(module => module.id)
                };
        ...
        // Advanced pricing and currency methods
    ...
    // Optional: Auto-initialize if all dependencies are met
    if (window.axios && document.readyState === 'complete') {
        try {
            const apiBaseUrl = 'https://18.215.160.136.nip.io/api';
            window.pricingManager = new PricingManager(apiBaseUrl);
        } catch (error) {
